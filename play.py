...
def red_choose(name, player, color_player_1 = None):
    back_button=Button.Button(10, 580, 40, white, goldMetalic, "Back")
    play_button = Button.Button(865, 580, 40, white, goldMetalic, "Play")
    s_red = slime.Slime(70, 200, "Red_Slime", 1.5)
    s_blue = slime.Slime(385, 200, "Blue_Slime", 1.5)
    s_green = slime.Slime(700, 200, "Green_Slime", 1.5)
    while True:
        Screen.blit(fix_brguide, (0, 0))
        s_red.draw(Screen)
        s_blue.draw(Screen)
...
def IPAdressText():
    ...
    Add_address_button = Button.Button(700, 300, 40, white, goldMetalic, "Add Server")
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if Add_address_button.clicked(mouse_position, event):
                server_thread = threading.Thread(target=lambda: subprocess.run(['python', 'server.py']))
                server_thread.start()
                red_choose("Multiplayer", 2)
                break
        ...
        if active:
            draw_caret(Screen, 325+ font.size(user_text[:caret_position])[0], 245, 32)
        if back_button.hover_over(mouse_position):
            back_button.blit_hover_over(Screen, black)
        else:
            back_button.blit(Screen, black)
        ...
        if Add_address_button.hover_over(mouse_position):
            Add_address_button.blit_hover_over(Screen, black)
        else:
            Add_address_button.blit(Screen, black)
